...
```{r}
plot(mytable$apsi,mytable$neff)
```
...
score_trad %>%
      left_join(myneff) %>% 
  left_join(mync) %>% 
  pivot_wider(id_cols=c(caseid,len,neff),names_from=method,values_from=SCI)# %>% 
trad_wider %>% 
  ggplot(aes (x=input,y=cmalign))+
...
with(trad_wider ,wilcox.test(cmalign,rnamrf,paired=TRUE))

with(trad_wider ,wilcox.test(cmalign,input,paired=TRUE))
with(trad_wider ,wilcox.test(rnamrf,input,paired=TRUE))
...
with(mytable.wide %>% filter(neff > 40),wilcox.test(`non-pair_rnamrf`,`non-pair_cmalign`,paired=TRUE,alternative = "greater"))
