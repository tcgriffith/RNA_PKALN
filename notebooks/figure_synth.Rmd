...
rslt.df.all=rslt.df.s %>% 
 select(method,caseid,contains("pk") & !contains("mcc")) %>% 
...
kable( booktabs = T,digits=2,
  ...
  kable_styling() %>%
  add_header_above(c(" ", "Pseudoknot alignment" = 3,"non-Pseudoknot alignment"=3,"Contact Prediction"=3)) %>%
...
gglists=lapply(1:length(testdirs.filt),function(i){
  ...
  gglist$rnamrf=plot_rscape(paste0(path,"/rnamrf/rnamrf.sorted.cov"))+
    ggtitle(label=caseid,subtitle="RNAmrf")
  return(gglist)
})
...
patch_rscape=
  wrap_elements(grob_ss1,ignore_tag=TRUE)+
  gglists[[1]]$rnamrf+gglists[[1]]$cmalign+
  wrap_elements(grob_ss2,ignore_tag=TRUE)+
  gglists[[2]]$rnamrf+gglists[[2]]$cmalign+
  wrap_elements(grob_ss3,ignore_tag=TRUE)+
  gglists[[3]]$rnamrf+gglists[[3]]$cmalign+
  wrap_elements(grob_ss4,ignore_tag=TRUE)+
  gglists[[4]]$rnamrf+gglists[[4]]$cmalign +
  plot_layout(byrow = FALSE,nrow=3,guides="collect",heights=c(1,3,3)) & theme_classic() & theme(legend.position="bottom") 
