...
int main(int argc, const char * argv[])
{
    ...
    string file_a;
    string file_b;
    vec_bool range_a; // position in a
    vec_bool range_b; // position in b
    bool use_gap_ss = false; // use ss penalty
    double gap_ss_w = 2; // gap penality weight at SSE 
    bool use_prf = false; // profile
    double prf_w = 1; // profile weight 
    double gap_open = -1; // gap open penalty
    double gap_ext = -0.01; // gap extension penalty
    int sep_cutoff = 3; // seq seperation cutoff ??
    int iter = 20;    //align iteration
    bool silent = false;
    ...
    // vec_a: columns with contacts
    // vec_a_div: number of contacts
    // vec_a_i: contacts of i
